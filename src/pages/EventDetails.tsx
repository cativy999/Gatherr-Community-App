...
const EventDetails = () => {
  ...
  // ── A/B variant: "rsvp_login_prompt" ──────────────────────────────────────
  // URL param ?ab_variant=A or B overrides (for previewing)
  // Otherwise random 50/50, stored in localStorage for consistency
  const abVariant: 'A' | 'B' = (() => {
    const urlParam = new URLSearchParams(window.location.search).get('ab_variant');
    if (urlParam === 'A' || urlParam === 'B') return urlParam;
    const stored = localStorage.getItem('rsvp_ab_variant');
    if (stored === 'A' || stored === 'B') return stored as 'A' | 'B';
    const assigned = Math.random() < 0.5 ? 'A' : 'B';
    localStorage.setItem('rsvp_ab_variant', assigned);
    return assigned;
  })();
  ...
  useEffect(() => {
    if (isGuest) { logAbEvent('prompt_shown'); }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Log login_completed when session goes from null → user (they just logged in)
  const prevSessionRef = useRef<boolean>(false);
  useEffect(() => {
    if (!prevSessionRef.current && !!session) { logAbEvent('login_completed'); }
  ...
  const ensureInviteLink = async (): Promise<string | null> => {
    ...
    if (!token) {
      const { data: created, error } = await supabase
        ...
        .select("id")
        .single();
      if (error || !created) { toast.error("Couldn't create an invite link."); return null; }
      token = created.id;
    }
    const link = `https://gatherr-one.vercel.app/cohost-invite/${token}`;
    setInviteLink(link);
    return link;
  };
          ...
          {isGuest ? (
            abVariant === 'A' ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <p style={{ textAlign:'center', fontWeight:700, fontSize:15, color:DARK, margin:0 }}>Join to save your spot 🎉</p>
                <button
                  onClick={(e) => { e.stopPropagation(); supabase.auth.signInWithOAuth({ provider:'google', options:{ redirectTo: window.location.href, queryParams:{ prompt:'select_account' } } }); }}
                  className="w-full py-3.5 rounded-full text-sm font-semibold text-white transition-opacity hover:opacity-80"
                  style={{ background: TEAL, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}
                >
                ...
                <button onClick={(e) => { e.stopPropagation(); requireAuth(); }} style={{ background:'none', border:'none', cursor:'pointer', color:TEAL, fontSize:13, fontWeight:600, textDecoration:'underline' }}>
          ...
          {isGuest ? (
            abVariant === 'A' ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                <p style={{ textAlign:'center', fontWeight:700, fontSize:14, color:DARK, margin:0 }}>Join to save your spot 🎉</p>
                <button
                  onClick={(e) => { e.stopPropagation(); supabase.auth.signInWithOAuth({ provider:'google', options:{ redirectTo: window.location.href, queryParams:{ prompt:'select_account' } } }); }}
                  className="w-full py-3 rounded-full text-sm font-semibold text-white transition-opacity hover:opacity-80"
                  style={{ background: TEAL, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}
                >
                ...
                <button onClick={() => requireAuth()} style={{ background:'none', border:'none', cursor:'pointer', color:TEAL, fontSize:12, fontWeight:600, textDecoration:'underline' }}>
                  or continue with email →
                </button>
              </div>
            )
